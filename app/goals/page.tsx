    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-400">{active.length} active · {completed.length} completed</p>
        </div>
        <AddGoalDialog />
      </div>
      ...
      {active.length === 0 && (
        <Card>
          <CardContent className="py-16 text-center text-zinc-500">
            ...
            <p className="text-lg font-medium text-zinc-300">No active goals</p>
      ...
      {completed.length > 0 && (
        <div>
          <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-3 flex items-center gap-2">
            <CheckCircle2 className="h-4 w-4 text-emerald-400" /> Completed Goals
          </h2>
            ...
            {completed.map((goal) => (
              <Card key={goal.id} className="opacity-70">
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-zinc-300">{goal.name}</p>
