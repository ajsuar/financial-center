  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div>
        ...
        <p className="text-sm text-zinc-400">{formatDate(now, "month-year")}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="col-span-1 lg:col-span-2">
          <CardHeader className="pb-2">
            <CardDescription className="flex items-center gap-1">
              Net Worth
              <span className={`text-xs ml-2 font-medium ${netWorthChange >= 0 ? "text-emerald-400" : "text-red-400"}`}>
          ...
          <CardContent>
            <div className="flex gap-4 text-sm text-zinc-400 mb-3">
              <span className="text-emerald-400">Assets {formatCurrency(netWorthData.totalAssets, "USD", true)}</span>
              <span>·</span>
              <span className="text-red-400">Liabilities {formatCurrency(netWorthData.totalLiabilities, "USD", true)}</span>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-xs text-zinc-500">
              Savings rate: <span className="text-zinc-300 font-medium">{summary.savingsRate.toFixed(0)}%</span>
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-xs text-zinc-500">
              Saved: <span className="text-emerald-300 font-medium">{formatCurrency(summary.savings)}</span>
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {activeGoals.length === 0 && (
              <p className="text-sm text-zinc-500 text-center py-4">No active goals</p>
            )}
            <Link href="/goals" className="block text-xs text-indigo-400 hover:text-indigo-300 text-center pt-1">
              View all goals →
            </Link>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {summary.byCategory.length === 0 && (
              <p className="text-sm text-zinc-500 text-center py-4">No expenses yet</p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-2">
            {recentTransactions.map((tx) => (
              <div key={tx.id} className="flex items-center justify-between gap-2">
                ...
                <span className={`text-sm font-medium shrink-0 ${tx.type === "INCOME" ? "text-emerald-400" : "text-red-400"}`}>
            ...
            <Link href="/transactions" className="block text-xs text-indigo-400 hover:text-indigo-300 text-center pt-1">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {accounts.map((acc) => (
              <div key={acc.id} className="flex items-center gap-3 p-3 rounded-lg bg-zinc-800/50 border border-zinc-800">
                <span className="text-xl">{acc.icon ?? "🏦"}</span>
                <div className="min-w-0">
                  <p className="text-xs text-zinc-400 truncate">{acc.name}</p>
                  <p className={`text-sm font-semibold ${acc.balance < 0 ? "text-red-400" : "text-zinc-100"}`}>
